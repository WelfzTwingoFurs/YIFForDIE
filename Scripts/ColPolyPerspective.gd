...
@export var linet = preload("res://Assets/test.png")
@export var line_size = Vector2(1,1)
@export var fillt = preload("res://Assets/test.png")
...
func _draw():
	...
	for v in $CollisionPolygon2D.polygon.size():
		...
		var color = 1
		if intense != 0:
			var diff = abs(rad_to_deg($CollisionPolygon2D.polygon[v].angle_to_point($CollisionPolygon2D.polygon[(v+1)%$CollisionPolygon2D.polygon.size()])) - light)
			if (diff > 180):
				diff = 360 - diff
			color = (diff/180)
		...
		if !Geometry2D.is_polygon_clockwise(poly):
			draw_colored_polygon(poly, Color(color,color,color), array_multiply(PackedVector2Array([Vector2(0,0), Vector2(1,0), Vector2(1,1), Vector2(0,1)]), top_size), topt)
	################################################################################################
	################################################################################################
